...
func (e *Carddeck) CreateCard(ctx context.Context, req *pb.CardRequest, rsp *pb.SuccessResponse) error {
	logger.Infof("Received Carddeck.CreateCard request: %v", req)
	deck, err := e.store.FindDeckByName(req.DeckName)
	if err != nil {
		rsp.Success = false
		return err
	}
...
func (e *Carddeck) CreateDeck(ctx context.Context, req *pb.DeckRequest, rsp *pb.SuccessResponse) error {
	logger.Infof("Received Carddeck.CreateDeck request: %v", req)
	roleRsp, err := e.collaborationService.GetGroupUserRole(context.TODO(), &pbcollab.GroupRequest{UserID: req.UserID, GroupName: req.GroupName})
	if err != nil {
		rsp.Success = false
		return err
	}
